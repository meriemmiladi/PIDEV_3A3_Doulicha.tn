...
public class DetailParticipationController implements Initializable {
    ...
    private void gererPart(MouseEvent event) {
        ...
        participation_evenement part= new participation_evenement();
           ...
           FXMLLoader loader = new FXMLLoader ();
                            loader.setLocation(getClass().getResource("/codingbeasts/doulicha/view/gererParticipation.fxml"));
                            try {
                                loader.load();
    ...
    if (now.isAfter(dateLimite)) {
        ...
        alert.setContentText("La date limite de modification a été dépassée puisqu'il reste moins de 48h avant l'évènement.");
        alert.showAndWait();
    }
    else { 
...
//            Stage window = (Stage) btn_modifierpart.getScene().getWindow();
    ...
    private void afficherPass(MouseEvent event) {
     ...
     Alert alert = new Alert(Alert.AlertType.CONFIRMATION);
        ...
        String filePath = "src/PDF/" + "participation_" + id_participationM.getText() + ".pdf";
...
//    }
    ...
    if (now.isAfter(dateLimite)) {
        ...
        alert.setContentText("La date limite de l'annulation est a été dépassée puisqu'il reste moins de 48h avant l'évènement.");
        alert.showAndWait();
    }
    else { 
         Alert alert = new Alert(Alert.AlertType.CONFIRMATION);
